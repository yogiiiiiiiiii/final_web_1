...
function handleRollNoFileUpload(event) {
    ...
    reader.onload = function(e) {
        ...
        studentInfo = lines.slice(1).map(line => {
            const [district, rollNo, candidateName] = line.split(',').map(item => item.trim());
            return { district, rollNo, candidateName };
        }).filter(info => info.rollNo !== '');
...
function generateNavigationPanel() {
    ...
    for (let student = 1; student <= totalStudents; student++) {
        const studentData = studentInfo[student - 1];
        const studentHeader = document.createElement('div');
        studentHeader.textContent = `${studentData.district} - ${studentData.rollNo} - ${studentData.candidateName}`;
